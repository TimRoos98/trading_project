...
forecast_df['value_change'] = forecast_df['value'].diff()
filtered_forecast_df = forecast_df[forecast_df['value_change'].abs() >= 500]
...
# Shade regions in the trades plot based on value changes in the forecast
for idx, row in filtered_forecast_df.iterrows():
    start_time = row['published_at_utc']
    end_time = start_time + pd.Timedelta(hours=1)
    if row['value_change'] > 0:
        ax1.axvspan(start_time, end_time, color='red', alpha=0.3)
    elif row['value_change'] < 0:
        ax1.axvspan(start_time, end_time, color='green', alpha=0.3)
